...
exports.getQuickStats = exports.searchHybrid = void 0;
...
const searchHybrid = async (req, res) => {
    try {
        ...
        const [matchingFiles, matchingEntities, matchingSnippets, matchingErrors] = await Promise.all([
            models_1.File.find({ userId, ...projectFilter, $or: [{ fileName: queryRegex }, { content: queryRegex }] }).limit(5),
            models_1.CodeEntity.find({ ...projectFilter, name: queryRegex }).populate('fileId').limit(5),
            models_1.Snippet.find({ userId, $or: [{ title: queryRegex }, { code: queryRegex }] }).limit(5),
            models_1.ErrorSolution.find({ userId, $or: [{ title: queryRegex }, { errorMessage: queryRegex }, { solution: queryRegex }] }).limit(5)
        ]);
        ...
        const finalResultsWithProjects = await Promise.all(finalResults.map(async (r) => {
            if (r.projectId) {
                const project = await models_1.Project.findById(r.projectId, 'name');
