...
exports.compareCodeStyle = exports.getDeveloperDNA = void 0;
const models_1 = require("../models");
const getDeveloperDNA = async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: 'Unauthorized.' });
        const userId = req.user.id;
        // Scan user's files to analyze DNA
        const files = await models_1.File.find({ userId });
        ...
        for (const proj of projects) {
            if (proj.framework)
                frameworksMap[proj.framework] = (frameworksMap[proj.framework] || 0) + 1;
            if (proj.database)
                databasesMap[proj.database] = (databasesMap[proj.database] || 0) + 1;
        }
...
exports.getDeveloperDNA = getDeveloperDNA;
const compareCodeStyle = async (req, res) => {
    try {
        const { code } = req.body;
        if (!code)
        ...
        const files = await models_1.File.find({ userId }, 'content');
        let userCamel = 0;
        let userSnake = 0;
        for (const file of files) {
...
exports.compareCodeStyle = compareCodeStyle;
