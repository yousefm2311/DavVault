...
class ParserService {
        ...
        for (const file of files) {
            ...
            if (file.path.endsWith('package.json')) {
                try {
                    ...
                    if (deps['mongoose'] || deps['mongodb'])
                        database = 'mongodb';
                    else if (deps['pg'] || deps['sequelize'])
                        database = 'postgresql';
            ...
            if (file.path.endsWith('pubspec.yaml')) {
                ...
                if (file.content.includes('supabase'))
                    database = 'supabase';
                if (file.content.includes('firebase'))
            ...
            if (file.path.endsWith('requirements.txt')) {
                ...
                if (file.content.includes('django'))
                    framework = 'django';
                else if (file.content.includes('flask'))
                    framework = 'flask';
                else if (file.content.includes('fastapi'))
                    framework = 'fastapi';
            }
        }
    ...
    parseJSOrTS(lines, content, entities) {
        // 1. Extract Classes
    ...
    parsePython(lines, content, entities) {
    ...
    parseDart(lines, content, entities) {
